...
namespace Controller.Camera
{
	...
	public class CameraController : MonoBehaviour {
		...
		public float CurrentUpperVerticalLimit { get; set; }
		[Range(0f, 90f)]
		...
		public float CurrentCameraSpeed { get; set; }
		...
		protected CameraControls cameraInput;

		//Setup references.
		void Awake () {
			tr = transform;
			cam = GetComponent<UnityEngine.Camera>();
			cameraInput = GetComponent<CameraControls>();

			cameraTarget.position = cameraBasePosition.position;
			...
			CurrentUpperVerticalLimit = upperVerticalLimit;
		...
		protected virtual void HandleCameraRotation()
		{
			...
			//Get input values;
			Vector2 _inputAxis = cameraInput.GetCameraAxis();
			float _inputHorizontal = _inputAxis.x;
			float _inputVertical = _inputAxis.y;
		...
		protected void RotateCamera(float _newHorizontalInput, float _newVerticalInput)
		{
			...
			currentXAngle += oldVerticalInput * CurrentCameraSpeed * Time.deltaTime;
			currentYAngle += oldHorizontalInput * CurrentCameraSpeed * Time.deltaTime;

			//Clamp vertical rotation;
			currentXAngle = Mathf.Clamp(currentXAngle, -CurrentUpperVerticalLimit, lowerVerticalLimit);
		...
		public void RotateTowardDirection(Vector3 _direction, float _lookSpeed)
		{
			...
			//Clamp vertical rotation;
			currentXAngle = Mathf.Clamp(_currentAngles.x, -CurrentUpperVerticalLimit, lowerVerticalLimit);
